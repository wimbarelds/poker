...
function runBenchmark() {
    ...
    const simulationDeck = createDeck().filter((card) => !knownCards.some((hcard) => hcard.id === card.id));
    ...
    for (let i = 0; i < POPULATION_SIZE; i++) {
        populationResults.push(randomOutcome({
            hand,
            community,
            deck: simulationDeck,
            otherPlayerMultipliers
    ...
    for (const sampleSize of SAMPLE_SIZES) {
        ...
        const variance = trialResults.reduce((acc, val) => acc + Math.pow(val - trialMean, 2), 0) / TRIALS_PER_SIZE;
        const stdDev = Math.sqrt(variance);
        
        const avgError = trialResults.reduce((acc, val) => acc + Math.abs(val - trueWinRate), 0) / TRIALS_PER_SIZE;

        console.log(`${sampleSize.toString().padEnd(10)} | ${stdDev.toFixed(4).padEnd(18)} | ${(avgError * 100).toFixed(2)}%`);
    }
}
